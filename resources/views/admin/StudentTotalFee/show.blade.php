   ...
   <div class="content-wrapper" style="min-height: 543px;">
        <!-- Content Header (Page header) -->
            ...
            <section class="content">
              <div class="container-fluid">
             <div class="card card-primary">
                    <div class="card-header">Allocate Fee Details  {{ $model->id }}</div>
                    <div class="card-body">
                       ...
                       <!--  <a href="{{ url('/admin/role') }}" title="Back"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</button></a>
                        <a href="{{ url('/admin/role/' . $model->id . '/edit') }}" title="Edit Role"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a> -->
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr><th> Student Roll NO. </th><td> {{ $model->rollno }} </td></tr>
                                    <tr><th> Student Name </th><td> {{ $model->student_name }} </td></tr>
                                    <tr><th> Academic Year/Semeter </th>
                                        <td> {{ $model->academic_year }}({{ $model->academic_tearm }})</td></tr>

                                     <tr><th>Category </th>
                                        <td> {{ $model->Feestructuredata->category }}</td></tr>    
					...
					<tr>
                                    <th> Extend Date </th>
                                    <td> {{  date('j \\ F Y', strtotime($model->extend_date)) }}
                                        &nbsp;&nbsp;&nbsp;&nbsp;
                                        <button type="button" class="btn btn-primary btn-sm editDate"
                                            data-id="{{$model->id}}" data-original-title="Edit" data-toggle="modal"><i
                    ...
                    @foreach($model->Feestructuredata->FeeDetails as $val)
                        <tr>
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="2">Total Amount</th>
                              <!-- <th colspan="1">{{ $model->totalamount }}</th> -->
                                <th colspan="1">{{ $model->totalamount }}</th>
...
$(function() {
    ...
    $('body').on('click', '.editDate', function() {
        ...
        //alert(fees_id);
        $.get("{{ url('/admin/studenttotalfee') }}" + '/' + fees_id + '/edit', function(data) {
    ...
    $('#saveBtn').click(function(e) {
        ...
        var url = "{{ url('/admin/studenttotalfee') }}" + '/' + fees_id ;
        $.ajax({
